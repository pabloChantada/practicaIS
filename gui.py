...
def open_file():
    ...
    return data
     # Elimina filas con NaN en cualquier columna
    data_cleaned = data.dropna()

    data_filled = data.fillna(value) 

    return data_cleaned  # o return data_filled
...
def create():
    ...
    if x_col.equals(y_col):
        ...
        # Elimina filas con NaN en x e y
        non_nan_indices = ~np.isnan(y_col_reshaped[:, 0])
        x_cleaned = x_col_reshaped[non_nan_indices, :]
        y_cleaned = y_col_reshaped[non_nan_indices, :]

        print("x_col_reshaped:", x_col_reshaped)
        print("y_col_reshaped:", y_col_reshaped)


        print("Número de NaN en X:", np.isnan(x_col.values).sum())
        print("Número de NaN en y:", np.isnan(y_col.values).sum())
